	...
	class Product extends Model
	{
		...
		public function setPhoto($file)
		{
			...
			switch ($extension) {
				case 'jpg':
				case 'jpeg':
					$image = (@imagecreatefromjpeg($file["tmp_name"])) === false ? imagecreatefrompng($file["tmp_name"]) : imagecreatefromjpeg($file["tmp_name"]);
		...
		public static function getPage($page = 1, $itemsPerPage = 10)
		{
			...
			$results = $sql->select("SELECT SQL_CALC_FOUND_ROWS * FROM tb_products ORDER BY desproduct
		...
		public static function getPageSearch($search, $page = 1, $itemsPerPage = 10)
		{
			...
			$results = $sql->select("SELECT SQL_CALC_FOUND_ROWS * FROM tb_products WHERE desproduct LIKE :search ORDER BY desproduct LIMIT $start, $itemsPerPage", [
				':search'=>'%'.$search.'%'
			]);
